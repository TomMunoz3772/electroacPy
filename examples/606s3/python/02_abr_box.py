...
This script shows how set up an ABR enclosure LEM/BEM simulation using pre-defined tools.
"""
...
mesh = "../geo/mesh_files/606_abr.msh"
...
system.lem_driver("LF", 1.0, Le, Re, Cms, Mms, Rms, Bl, Sd)
system.lem_enclosure_2("LF_abr",      # name used as a reference for simulations
                       Vb=13.56e-3,         # enclosure volume in m3
                       Mmd=30.91e-3,        # moving mass
                       Cmd=790e-6,          # suspension compliance
                       Rmd=0.57,            # mechanical losses
                       Sd=143.1e-4,         # ABR radiating surface
                       setDriver="LF",      # need to set lem_driver into enclosure
                       ref2bem=[1, 2])      # reference to BEM groups, see mesh_from_python file, -> abr comes last

# STUDY SETUP
system.study_exteriorBEM("free-field",          # name of study, multiple studies can be defined
                         meshPath=mesh,               # self-explanatory
                         acoustic_radiator="LF_abr",  # radiators used in simulation, must be referenced to BEM boundaries
...
# save results
ep.save("saved_data/02_ABR", system)
