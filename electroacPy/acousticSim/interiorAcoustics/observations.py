...
class observations:
    ...
    # def addSphericalObservation(self, obsName: str, nMic: int = 256, radius: float = 1.8):
    #     # check if observation exists
    #     isComputed = self.checkObservationExists(obsName, deleteObservation=True)
    #
    #     if isComputed is False:
        ...
        if type(obs_name) == list:
            ...
            for i in range(nObs):
                ...
                if self.observationType[idxTmp] == 'polar':
                    ...
                    cursor = plotTotalDirectivity_v2(theta,
                                         self.bemObject.freq_array, micData[pMicIdx], title=self.observationName[idxTmp])
                    out.append(cursor)

                elif self.observationType[idxTmp] == 'spherical':
                    boxSphere = plotSphericalRadiation(self.xMic[idxTmp], micData[pMicIdx], self.bemObject.freq_array)
                    out.append(boxSphere)

                elif self.observationType[idxTmp] == 'planar':
                    # print('current observation: ', self.observationType[idxTmp])
                    # print('add2plotter: ', self.add2Plotter[idxTmp])
    ...
    def plot_system(self, engineering=False):
        ...
        pl = pyvista.Plotter()
        pl.add_mesh(mesh, show_scalar_bar=False, color='grey', opacity=0.15)
        ...
        for i in range(len(self.xMic)):
            point_cloud_tmp = pyvista.PolyData(self.xMic[i])
    ...
    def save(self, filename: str):
        """
        Save the computed observations in a npz archive.
        :param filename: str
                    Output file name (.npz)
        :return None:
        """

        # need to reshape pMic because of non-regular arrays
        npmicList = len(self.pMic)      # number of observations
        maxList= np.zeros(npmicList)    # max number of microphone per observations (empty array)
        for i in range(npmicList):
            maxList[i] = np.shape(self.pMic[i])[1] # max number of microphones per observations

        # set mic arrays
        maxMic = np.max(maxList) # give the maximum number of microphones along the defined observations
        xmicArray = np.zeros([int(maxMic), 3, npmicList])

        # set pressure arrays
        pMic = np.zeros([len(self.bemObject.freq_array),
                         int(maxMic),
                         npmicList], dtype=complex)
        pMicArray = np.zeros([len(self.bemObject.freq_array),
                              int(maxMic),
                              len(self.bemObject.radiatingSurface),
                              npmicList], dtype=complex)

        for i in range(npmicList):
            xmicArray[:int(maxList[i]), :, i] = self.xMic[i]
            pMic[:, :int(maxList[i]), i] = self.pMic[i]
            pMicArray[:, :int(maxList[i]), :, i] = self.pMicArray[i]


        # bem p_total_coeff
        p_array_coeff = False
        bem = self.bemObject
        if bem.isComputed is True:
            # pressure data
            p_array_coeff = np.zeros([len(bem.freq_array), bem.nRad_S, bem.vertices], dtype=complex)

            # velocity data
            verticesU = len(bem.grid.domain_indices)
            # u_tot_coeff = np.zeros([len(freq_array), vertices], dtype=complex)

            for f in range(len(bem.freq_array)):
                for rs in range(bem.nRad_S):
                    p_array_coeff[f, rs, :] = bem.p_total_list_array[f, rs].coefficients
        ...
        # bem related data
        freq_array              = self.bemObject.freq_array,
        meshPath                = self.bemObject.meshPath,
        # sizeFactor              = self.bemObject.sizeFactor,
        vertices                = self.bemObject.vertices,
        p_array_coeff           = p_array_coeff,

        # general data
        observationName         = np.array(self.observationName),
        observationType         = np.array(self.observationType),
        computedObservations    = self.computedObservations,
        xMic                    = xmicArray,
        labels                  = np.array(self.labels, dtype=object),
        radiatingSurface        = self.bemObject.radiatingSurface,

        # planar observations
        L                       = np.array(self.L, dtype=object),
        W                       = np.array(self.W, dtype=object),
        planarPlane             = np.array(self.planarPlane, dtype=object),
        add2plotter             = np.array(self.add2Plotter, dtype=object),

        # polar observations
        theta                   = np.array(self.theta, dtype=object),
        polarPlane              = np.array(self.polarPlane, dtype=object),
        # DI                      = np.array(self.DI),
...
def createSphereArray(nMic, sphereRadius=1.8):
    """
...
def plotTotalDirectivity_v2(theta, freq, pMic, xscale='log', fmin=20, fmax=20e3,
    ...
    gca2 = ax2.contourf(freq, np.rad2deg(theta), directivity.T, np.arange(-20, 2, 2), cmap='turbo')
    ax2.set_xscale('log')
...
def plotPressureField_pyvista(bemOBJ, toPlotter, toPlotterMicIdx, L, W, pMicData, xMic, radiatingSurface):
    ...
    for i, idxData in enumerate(toPlotterMicIdx):
        ...
        meshPlot[i].points = np.array(xMic[idxData], dtype=float)
        meshPlot[i].dimensions = [len(L[idxData]), len(W[idxData]), 1]


    engine = pyvista_spkPlotter(pl, bemOBJ, toPlotter, toPlotterMicIdx, L, W, pMicData, xMic, radiatingSurface)
...
#     p_total_list_array = bemObj.p_mesh
...
#         for j in range(len(radiatingSurface)):
#             pressureCoeff[f, :] += p_total_list_array[f][j].coefficients
...
def sumPressureArray(bemObj, radiatingSurface):
    ...
    pressureCoeff = np.zeros([len(bemObj.freq_array), bemObj.vertices], dtype='complex')
    grid_dof = bemObj.spaceP.grid_dof_count
    for f in range(len(bemObj.freq_array)):
        for j in range(len(radiatingSurface)):
            ind_surface = np.argwhere(radSurf_system == radiatingSurface[j])[0][0]
            pressureCoeff[f, :grid_dof] += p_mesh[f][ind_surface].coefficients
    return pressureCoeff
...
class pyvista_spkPlotter:
    def __init__(self, mesh, bemOBJ, toPlotter, toPlotterMicIdx, L, W, pMicData, xMic, radiatingSurface):
        ...
        self.grid_dof = bemOBJ.spaceP.grid_dof_count
    ...
    def update(self):
        ...
        for i, idxData in enumerate(self.toPlotterMicIdx):
            ...
            # system mesh
            print("len pressureCoeff_system::: ", np.shape(pressureCoeff_system))
            self.output.remove_actor('spk_mesh')
            self.output.subplot(0, 0)
            self.output.add_mesh(self.speaker, show_edges=showMesh,
                        scalars=pressureCoeff_system[freq_idx, :], cmap=cmap_d,
                        n_colors=ncolor,
