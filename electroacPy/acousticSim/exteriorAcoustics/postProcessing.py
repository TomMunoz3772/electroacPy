...
import numpy as np
from electroacPy.acousticSim.exteriorAcoustics.observations import observations
        ...
        for pMicIndex in range(len(pMicArray_update)):
            for i, val in enumerate(radSurf_bem):
                if val in radiatingSurface:
                    # print('indice of surface to process: ', i)
                    # print('surface to process: ', radSurf_bem[i])
...
def buildPostProcessedObservation(observationObject, # observation data (not processed)
    ...
    boundary            = observationObject.bemObject.boundary
    offset              = observationObject.bemObject.offset
    ...
    bemObject = bem_ext(meshPath, radSurf, surfVelocity, freq_array, vibrometry_points,
                        radiation_direction, boundary, offset, c_0=c_0, rho_0=rho_0)
