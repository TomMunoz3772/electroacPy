...
class loudspeakerSystem:
    ...
    def lem_enclosure(self, name, Vb, eta=1e-5, setDriver=None, Nd=1, wiring="parallel", ref2bem=None, **kwargs):
        ...
        if setDriver != None:
            physics.getDriverResponse(self.driver[setDriver], Nd=Nd, wiring=wiring)
    ...
    ## ==================
    # %% ACOUSTIC STUDIES
    def study_radiationBEM(self, name, meshPath, acoustic_radiator, 
        ...
        for xover in ls.crossover:
            if ls.crossover[xover].referenceStudy == study:  # check if some of the crossovers are to be applied in the study
                ...
                if isinstance(ref2bem, int):
                    ls.results[study].addTransferFunction("filter_stage", h, [ref2bem])
...
def apply_Velocity_From_SPKBOX(loudspeakerSystem, study, radiatorName):
    ...
    if isinstance(ref2bem, int) is True:
        if ls.enclosure[radiatorName].config == "sealed":  # check if 1 radiator because sealed or bp config
            ls.results[study].addTransferFunction("driver", v, [ref2bem])
        elif ls.enclosure[radiatorName].config == "bandpass":
            ls.results[study].addTransferFunction("port", vp, [ref2bem])
        elif ls.enclosure[radiatorName].config == "bandpass_pr":
            ls.results[study].addTransferFunction("passive-radiator", vpr, [ref2bem])
        ...
        if ls.enclosure[radiatorName].config == "sealed":
            ...
            ls.results[study].addTransferFunction("driver", v, ref2bem[:-1])
            ls.results[study].addTransferFunction("port", vp, [ref2bem[-1]])
        elif ls.enclosure[radiatorName].config == "passiveRadiator":
            ls.results[study].addTransferFunction("driver", v, ref2bem[:-1])
            ls.results[study].addTransferFunction("passive-radiator", vpr, [ref2bem[-1]])
        elif ls.enclosure[radiatorName].config == "bandpass":
            ls.results[study].addTransferFunction("port", vp, ref2bem)
        elif ls.enclosure[radiatorName].config == "bandpass_pr":
            ls.results[study].addTransferFunction("passive-radiator", vpr, ref2bem)
...
def apply_Velocity_From_PLV(loudspeakerSystem, study, radiatorName):
    ...
    if isinstance(ref2bem, int) is True:
        ls.results[study].addTransferFunction("unit_v", v, [ref2bem])
    elif isinstance(ref2bem, list) is True:
        ls.results[study].addTransferFunction("unit_v", v, ref2bem)
    return None
